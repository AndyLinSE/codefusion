...
**Purpose:**  
Create a desktop app that lets you drag-and-drop a codebase folder onto a window. The app will recursively traverse the folder, read supported code files (extensions: `.js`, `.json`, `.md`, `.ts`, `.html`, `.css`, `.rules`, `.py`), and combine them into one annotated text file for AI analysis. Annotations include each file’s folder and name. In addition, the app will:

- **Omit Files/Folders:**  
  By default, omit files/folders matching patterns such as those in a `.gitignore` file, folders like `.git` and `node_modules`, and any file/folder that starts with a period (e.g., `.next`, `.swc`).
  
- **Interactive Preview:**  
  Provide a UI preview that lists files to be processed and those that will be omitted—giving you the chance to adjust the omission settings interactively via extension filters and regular expressions.
  
- **Output Statistics:**  
  Show the total number of characters in the combined text file and an approximate token count (using the character count divided by 4).

- **Packaging:**  
  Package the application as an installer for Windows.
...
### 3.1 File Handling and Recursion
...
### 3.2 Combining Files and Generating Statistics
- **Combining Logic:**  
